...
export default function CategoryPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const filteredProducts = products.filter(p => p.category.toLowerCase() === formattedCategory.toLowerCase());
    const baseProducts = filteredProducts.length > 0 ? filteredProducts : products;

    const displayProducts = [...baseProducts].sort((a, b) => {
        switch (sortBy) {
            case "price-asc": return a.price - b.price;
            case "price-desc": return b.price - a.price;
            case "popular": return (b.isBestSeller ? 1 : 0) - (a.isBestSeller ? 1 : 0);
            default: return 0; // newest = API order
        }
    });
    ...
    if (loading) {
        return (
            <main className="min-h-screen bg-background flex items-center justify-center">
                <Loader2 className="w-10 h-10 text-primary-900/30 animate-spin" />
            </main>
        );
    }
                ...
                <div className="container mx-auto px-4 lg:px-8 relative z-10 flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
                    <div className="flex flex-col gap-6 w-full md:w-2/3">
                        ...
                        <div className="flex items-center gap-2 text-[10px] sm:text-xs font-mono tracking-[0.2em] text-[#fcfcfc]/50">
                            ...
                            <span className="text-danger">/</span>
                            <span className="text-white border-b border-white pb-0.5">{formattedCategory.replace(" ", "_").toLocaleUpperCase('en-US')}</span>
                        </div>
                        ...
                        <div className="relative inline-block group cursor-pointer">
                            <motion.h1
                                ...
                                className="font-display text-4xl sm:text-6xl md:text-7xl lg:text-[7rem] leading-[0.85] tracking-tight relative z-10 group-hover:opacity-0 transition-opacity duration-300"
                            >
                                {formattedCategory.toLocaleUpperCase('en-US')}<span className="text-danger">.</span>
                            </motion.h1>

                            {/* Hover Half-and-Half Neon Outline */}
                            <span
                                className="font-display text-4xl sm:text-6xl md:text-7xl lg:text-[7rem] leading-[0.85] tracking-tight absolute inset-0 z-20 opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-700 neon-hover-static"
                                style={{
                            ...
                            >
                                {formattedCategory.toLocaleUpperCase('en-US')}<span className="text-transparent">.</span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="w-full md:w-1/3 flex flex-col items-start md:items-end text-left md:text-right" style={{ justifyContent: 'flex-end', height: '100%' }}>
                        ...
                        <p className="text-sm md:text-base font-medium text-white/70 max-w-sm uppercase tracking-wide">
                            Explore our curated selection of {formattedCategory}. Premium materials, uncompromising cuts, engineered for the streets.
                        </p>
                    </div>
                </div>
            </div>
                    ...
                    <div className="flex-1 w-full relative">
                        ...
                        <div className="sticky top-16 lg:top-[72px] z-30 bg-background/90 backdrop-blur-md border-b border-[#1a1a1a]/10 px-0 py-4 flex items-center justify-between mb-6">
                            ...
                            <div className="hidden lg:block text-sm text-primary-900/50 font-medium">
                                Showing <span className="text-primary-900 font-bold">{displayProducts.length}</span> Results
                            </div>
